...
class bin_search_tree
{
   ...
   public  :
	    ...
	    void check()
	    {
			...
			if( x == 1)
			{
				cout<<" same number of nodes"<<endl;
			}
			else
			{
				cout<<"notttt"<<endl;
		...
		int nodes(node *temp)
		{
			...
			return(nodes(temp->left) + nodes(temp->right) + 1);
		}
		int trav(node *temp)
		{
			int R =0;
    ...
    switch(ch)
     {
       case '1':
		...
		case '3':
		system("cls");
		cout << "if 1 yes else no\n" << obj.trav(obj.root) << endl;
       case '4':
		exit(0);
		break;
       default :
...
void bin_search_tree :: options()
{
 ...
 cout<<"\n 3 : CHECK IF SAME NO. OF NODES ON LEFT AND RIGHT.\n";
 cout<<"\n 4 : QUIT PROGRAM. \n";
}//--------------------------------------------------------------
